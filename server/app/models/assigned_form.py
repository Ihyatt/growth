...
class AssignedForm(db.Model):
    ...
    # form_author = relationship('User', foreign_keys=[form_template_id], backref='forms_as_practitioner', lazy='select')

    form_template = relationship('FormTemplate', backref='patient_forms', lazy='select')
    ...
    def __repr__(self) -> str:
        return f"<AssignedForm id={self.id} patient={self.patient_id} form={self.form_template_id} status={self.status.name if self.status else None}>"