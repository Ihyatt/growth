...
class User(db.Model):
    ...
    # Relationships - update model names to actual ones in your codebase
    medications = relationship('Medication', back_populates='patient_owner', lazy=True)
    medication_comments = relationship('Comment', back_populates='medication_comment_author', lazy=True)
    report_comments = relationship('Comment', back_populates='report_comment_author', lazy=True)
